...
When using LDAP authentication the following additional variables become available. All DB* variables will not be used in this case:

| Variable | Description | Type | Default value |
| -------- | ----------- | ---- | ------------- |
| **LDAP_ENABLED** | Use LDAP authentication | *optional* | false
| **LDAP_HOST** | The LDAP host server | *optional* | ldap
...
To enable LDAP you have to set **LDAP_ENABLED** to *true*. All **DB\*** enviroment variables will not be used in this case (all become optional).

The LDAP server has to be specified with **LDAP_HOST**. Note that as with the SQL servers, the LDAP server will be resolved and hard coded in the /etc/hosts file on startup for faster lookups. Make sure the LDAP server is resolvable and available at conainer startup or the startup sequence will fail!
