...
USE_TRAEFIK_CERT=${USE_TRAEFIK_CERT:-false}
...
if [ "$USE_TRAEFIK_CERT" = true ]; then
  echo "[INFO] Search for SSL certificates generated by Traefik"
  while [ ! -f "$ACME_PATH"/acme.json ]
  do
    sleep 5
    echo "[INFO] ..."
  done
  echo "[INFO] SSL certificates found, dumping into pem files"
  mkdir -p /tmp/ssl "$LETS_ENCRYPT_LIVE_PATH"
  dumpcerts.sh "$ACME_PATH"/acme.json /tmp/ssl >/dev/null
  mv -f /tmp/ssl/certs/"$FQDN".crt "$LETS_ENCRYPT_LIVE_PATH"/fullchain.pem
  mv -f /tmp/ssl/private/"$FQDN".key "$LETS_ENCRYPT_LIVE_PATH"/privkey.pem
  rm -rf /tmp/ssl
else
